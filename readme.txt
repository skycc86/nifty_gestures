...
; powerful screen capture funtion	-  Credited to sreen capture script by Sean
...
; Added Top 5 favourite Apps


; **** some shortcut may miss out and left undocumented
...
; added tray indicator (optional)
; remove GestureKey toggle suspend
; remove some unnecessary tray menu item
; activate window under mouse cursor so that drawing gesture take effect on mouse underneath window
; added back the load Gestures.ini function
; change gesture default key to MButton, left RButton for Nifty Drag function
; added black list apps for nifty dragging function
; added exception on remote desktop and VNC window 
; added reload settings at tray options
; added disable option for editor adv copy
; fixed window placement to work on second monitor, added more placement option (#! arrow key combination)
; added condition for adobe reader and word browsing gesture
; 20110812 - make gesture working for blacklist_apps if click and draw gesture start from caption

; Version 1.2 (win7 - should work for XP as well)
; change default gesture up for explorer to alt+up
; increase favourite to 25
; add option to tray icon for vim copy_paste, start with windows and Gesture Enable
; vim copy when select from left -> right / selected text with double click in notepad/notepad++
; map capslock key to tab, use shift+capslock / ctrl+capslock for capslock
; added remote desktop to nifty right and middle button blacklist
; compile readme files and icons inside exe
; map AppsKey to my favourite
; defeature Miranda shorcuts which are not used
; update new ahk_class for VNC windows
; added drawing mode, us

; Ver 1.21
; added Win + RButton for alternate access to some features
; include browser for vim copy feature
; added google chrome to browser class

####################################################
my_NiftyWindows.ahk		-->  my_NiftyWindows.ini
	->general.ahk
	->my_shortcut.ahk
		->ScreenCapture.ahk
	->Gestures.ahk		--> Gestures.ini
		->Default_Gestures.ahk

; Ver 1.22
; added word + ppt to editor group
; added multiclip function from skrommel script

; Ver 1.23
; allow variable of Favourite
; CapsLock + arrow for navigation
; double CapsLock shortcut
; allow run with run_*
; swap win + drag with shift + drag, remove win + drag feature
; ctrl + space toggle taskbar
; win + x for window pack/unpack
